/*
 * proxy.c - A Concurrent Web proxy (Multi-Processing)
 * Name: Calvin C Chiu
 * 
...
int main(int argc, char **argv)
{
	...
	printf("proxy: In Child Process\n");
		...
		bzero(buf, MAXLINE);
	    	...
	    	/*
	     	* Lock the file when information is being logged
	     	*/	
	    
		flock(log_entry, LOCK_SH);
		format_log_entry(log_entry, &clientaddr, request_uri, response_len);  
		fprintf(log_file, "%s %d\n", log_entry, response_len);
		fflush(log_file);
		flock(log_entry, LOCK_UN);
		
		/*
		* Unlock when logging is finished
		*/
	    ...
	    close(serverfd);
	    free(request);
	    printf("proxy: Exit Child process\n");

	    }
	    else if(pid > 0){
	    	printf("proxy: In Parent Process\n");
